...
def init(self, training_dir):
    ...
    :param training_dir: path of training directory with subdirectories '/ham'
    and '/spam'
    """
...
self.uniq_h_toks = len(self.ham_table) self.uniq_s_toks =
len(self.spam_table) self.total_h_toks =
sum(self.ham_table.values()) self.total_s_toks =
sum(self.spam_table.values())
self.tok_arr = sorted(list(self.ham_table.keys()) + list(self.spam_table.keys()))
self.freq_tab = self.create_frequency_table() self.file_count = 0
self.count_spam = 0 self.count_ham = 0 self.spam_list =[] self.ham_list =[]
...
for tok in self.tok_arr:
    entry = {}
s_freq =
self.spam_table.get(tok, 0)
entry['spam_freq'] = s_freq
h_freq
= self.ham_table.get(tok, 0)
...
def prob_spam(self, token):
    ...
    """
val = self.freq_tab.get(token) if
val is not None:
...
def classify(self, filepath):
    ...
    """classifies a file as spam or ham based on training data :param
    filepath:
    :return: (boolean) True->spam, False->ham
    """
self.file_count += 1 if self.prob_msg_spam(filepath) >
self.prob_msg_ham(filepath): self.count_spam += 1
...
try:
    for f in os.listdir(dir_path): self.classify(dir_path + f)
if known_type == 'spam': correct =
self.count_spam / float(self.file_count)
else:
...
def clean_table(self, min_freq):
    ...
    """Removes entries from frequency table if they are deemed poor indicators. or
    if combined spam/ham frequency is below 'min_freq'
...
if (v['spam_freq'] + v['ham_freq'] < min_freq or
    ...
    rm_keys.append(k)
for k in rm_keys: print("deleting " + str(k) + " from freq
table in clean()
") del self.freq_tab[k]
...
spam
e - mails: {:22
d}'.format(len(os.listdir('
emails_small / testing / spam / '))))
...
toks.extend(words)
except FileNotFoundError as e: print("Error:"
                                     + str(e))

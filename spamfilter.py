...
class Spamfilter():
    ...
    def init (self, training_dir):
        """ inits Spamfilter with training data :
            param training_dir: path of training directory
            with subdirectories '/ham' and '/spam' """
    ...
    def create_frequency_table(self):
        ...
        :return: dict{k,v}: spam/ham frequencies k = (str)token, v =
    ...
    def prob_spam(self, token):
        """calculates the probability that 'token' is found in spam emails :param token: (str)
    ...
    def prob_ham(self, token):
        """calculates the probability that 'token' is found in ham emails :param token: (str)
    ...
    def prob_msg_spam(self, filepath):
        """Calculates the probability that a message is spam :param filepath: (str) path of email
    ...
    def prob_msg_ham(self, filepath):
        """Calculates the probability that a message is ham :param filepath: (str) path of email
    ...
    def classify(self, filepath):
        """classifies a file as spam or ham based on training data :param filepath:
    ...
    def classify_all(self, dir_path, known_type='spam'):
        """Classifies all emails in a testing directory and maintains count of errors :param dir_path: path of testing directory
    ...
    def clean_table(self, min_freq):
        """Removes entries from frequency table if they are deemed poor indicators. or if combined spam/ham frequency is below 'min_freq'
    ...
    def tokens(text, tok_size=3):
        """ Returns a list of all substrings contained in 'text' of size 'tok_size' :param text: (string) text to tokenize
        :param tok_size: length of substrings
        :return: (list) tokens of 'text'
        """
        return [text[i:i
 + tok_size]
                for i in range(len(text) - tok_size + 1)]

    def clean_split(in_str):
        """ Removes all non-alphanum chars and splits string at whitespace, downcase :param in_str: (str) target string
        :return: (list) cleaned strings
        """
        return re.sub(r'[^\s\w]|_', '', in_str).lower().split()

    def file_tokens(filepath):
        """ tokenizes all strings contained in 'filepath' after removing all
        non-alphanum chars and splitting strings at whitespace
        :param filepath: path of target file :return: list of tokens
        """
        toks=[]
        try:
    ...
    def dir_tokens(dir_path):
        ...
        :param dir_path: directory containing files to be tokenized :return: list of tokens
        """
        dir_toks=[]
        try:
            filenames = os.listdir(dir_path)
            for f in filenames: dir_toks.extend(file_tokens(dir_path + f))
        except FileNotFoundError as e:
            print("Error:" + str(e))
        return dir_toks
    if _name=="main_":
        spam_filter= Spamfilter('emails_small/training/')
        spam_filter.print_table_info()
        spam_filter.classify_all("emails_small/testing/spam/",   'spam')
        spam_filter.classify_all("emails_small/testing/ham/", 'ham')
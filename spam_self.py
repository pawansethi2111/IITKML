...
class Spamfilter():
    ...
    def __init__(self, training_dir):
        ...
        self.ham_table = dict(Counter(self.dir_tokens(training_dir + "ham/")))
        print('Training filter with known spam...')
        self.spam_table = dict(Counter(self.dir_tokens(training_dir + 'spam/')))
    ...
    def prob_msg_spam(self, filepath):
        toks = self.file_tokens(filepath)
        sum = 0
        for tok in toks:
    ...
    def clean_table(self, min_freq):
        ...
        for k, v in self.freq_tab.items():
            if (v['spam_freq'] + v['ham_freq'] < min_freq or 0.45 < (
                    v['prob_spam'] / (v['prob_spam'] + v['prob_ham'])) < 0.55):
                rm_keys.append(k)
        for k in rm_keys:
    ...
    def clean_split(self,in_str):
        return re.sub(r'[^\s\w]|_', '', in_str).lower().split()

    def file_tokens(self,filepath):
        toks = []
        try:
            with open(filepath, encoding="utf8", errors='ignore') as fp:
                for line in fp:
                    words = self.clean_split(line)
                    toks.extend(words)
        except FileNotFoundError as e:
            print("Error:" + str(e))
        return [x for x in toks if len(x) < 10]

    def dir_tokens(self,dir_path):
        dir_toks = []
        try:
            filenames = os.listdir(dir_path)
            for f in filenames:
                dir_toks.extend(self.file_tokens(dir_path + f))
        except FileNotFoundError as e:
            print("Error:" + str(e))
        return dir_toks

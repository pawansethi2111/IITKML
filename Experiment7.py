...
df = df[['Adj. Close']]
df.fillna(-99999, inplace=True)  # 1) what
happens if I
take
this
out?
forecast = int(math.ceil(0.01 * len(df)))  # 1% of stocks lifetime to be forecasted df['Forecast']
= df['Adj. Close'].shift(-forecast)  # 2) figure out correct shifting model
'''
if this shift is used,
[1% of stocks lifetime] (e.g. 33) days ago is the forcasted price for that day. this
shift is built to learn from the stocks trend.
'''
...
with open('prediction.pickle', 'wb') as f: pickle.dump(clf,
                                                       f)
...
# RIGHT NOW: Data is just shifting back 5 places as seen in forecast variable df['Forecast']
= np.nan
